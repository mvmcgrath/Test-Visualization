...
public class BTraceGen extends Report {
    ...
    final String RUNNER_PATH = "../generator/app/src/main/java/com/github/mvmcgrath/generator/core/Runner.java";

    public void generateBTraceReports() {
        HashMap<String, ArrayList<String>> classMap = findTestCases();

        String[] commands = new String[]{"#!/bin/bash", "cd ../generator/btrace/config", "rm -r results", "mkdir results"};
    ...
    public void generateSingleReport(String className, String testName) {
        ...
        String[] commands = new String[]{"#!/bin/bash", "cd ../generator", "bash ./gradlew shadowJar",
                "cd btrace/config", "java -javaagent:../libs/btrace-agent.jar=scriptOutputFile=" + className + "." + testName +
                ".txt,script=BTraceConfig.class -jar app-all.jar", "mv " + className + "." + testName + ".txt results"};
        executeBashScript(commands);
    }
